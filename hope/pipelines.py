...
class HOpePipeline:
    ...
    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `hope`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM hope WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO hope (date, title, link, content)"
